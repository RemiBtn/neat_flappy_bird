...
class Bird:
    ...
    def move(self):
        ...
        d = self.tick_count*self.vel + 0.5*(3)*(self.tick_count)**2
...
class Pipe:
    ...
    def set_height(self):
        self.height = random.randrange(50, 275)
...
def draw_window(win, birds, pipes, base, score, gen):
    # Display all the elements for the game
...
def main(genomes, config):
    # Fitness function that runs the game for the IA and allow it to compute the fitness for every bird
    ...
    run = True
    while run:
        clock.tick(30)  # actualisation every 30ms
        ...
        if add_pipe:
            score += 1
            for g in ge:
                g.fitness += 5  # because we will already have deleted the genomes of the death birds
...
def run(config_path):
    # Launch the training of the neural network
...
def run_trained_bird(config_path, genome_path="winner.pkl"):
    # Make a bird already trained play the game
